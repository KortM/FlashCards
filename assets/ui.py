...
import flet
from flet import Container, ListView, colors, border, ButtonStyle
...
class AdminUI(UserControl):
    ...
    def build(self):
                ...
                questions.controls.append(
                    Row(
                        ...
                        controls=[
                            ...
                            FloatingActionButton(text='',icon=icons.DELETE, width=32, height=32)
                        ]
                    )
                )
        ...
        btn_add = FloatingActionButton(on_click=new_term, icon=icons.ADD, height=42, width=64, bgcolor=colors.WHITE)
            ...
            controls=[
                Column(
                    ...
                    Row(
                        alignment='center',
                        controls=[
                            FilledButton(text='Сменить пользователя', icon=icons.SWITCH_ACCOUNT)
                        ]
                    ),
                    ]
                ),
...
def main(page: Page):
    ...
    passwd_view = TextField(label='Пароль', width=300,
                            password=True, border='outline')
    login_btn = FilledButton(text='Войти', width=300, icon=icons.LOGIN, style=ButtonStyle(color='blue'), height=60)
